...
function buildLineString(fea, ctx) {
  const arcs = getPrimitives(fea.fullId, ctx)
    .map(f => ctx.arcs[f])
  if (arcs.length === 0) {
...
function buildPoint(fea, ctx) {
  const nodes = getPrimitives(fea.fullId, ctx)
