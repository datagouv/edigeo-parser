...
import {deintersect, fixHasExteriorHoles} from './geometry/clean.js'
...
function buildFeature(feaBlock, ctx) {
  ...
  if (featureType in featureTypes) {
    ...
    try {
      ...
      if (errors.length === 0) {
      ...
      } else if (errors.length === 1 && errors[0] === 'has-exterior-holes') {
        const facesIds = ctx.getPrimitives(feaBlock.fullId)
        const faces = facesIds.map(primitiveId => ctx.faces[primitiveId])

        // eslint-disable-next-line unicorn/no-array-reduce
        const features = faces.reduce((acc, curr) => {
          const faceFullId = curr.fullId
          acc = [...acc, ...curr.arcs.map(arc => ({
            type: 'Feature',
            properties: {
              id: arc.id,
              fullId: arc.fullId,
        ...
        const geojsonFeatureLines = {
          type: 'FeatureCollection',
          features,
        }

        const fixed = fixHasExteriorHoles(geojsonFeatureLines)
        setGeometry(feature, fixed.features[0].geometry, ctx.proj)
      } else {
        console.log(errors)
      ...
      if (geometryType === 'LineString') {
        ...
        const facesIds = ctx.getPrimitives(feaBlock.fullId)
        const debugFeature = facesIds.map(primitiveId => ctx.faces[primitiveId])
        if (debugFeature && facesIds) {
          // eslint-disable-next-line unicorn/no-array-reduce
          const features = debugFeature.reduce((acc, curr) => {
            const faceFullId = curr.fullId
            acc = [...acc, ...curr.arcs.map(arc => ({
              type: 'Feature',
              properties: {
