...
const {truncate, rewind} = require('@turf/turf')
...
function rejectGeometry(feature, errors) {
  ...
  if (featureType in featureTypes) {
    ...
    try {
      ...
      if (errors.length === 0) {
        ...
        rejectGeometry(feature, errors)
      }
    } catch (err) {
      console.error(`${ctx.bundle ? ctx.bundle + ' | ' : ''}${layer}:${feaBlock.id} => geometry ignored (${err.message})`)
      rejectGeometry(feature, [err.message])
    }
