import {kinks, polygon, booleanWithin, intersect} from '@turf/turf'
import {first, last, uniq, flatten} from 'lodash-es'
...
function validateMultiPolygon(geometry) {
  const errors = uniq(
    flatten(
      geometry.coordinates.map(coords => validatePolygonRings(coords)),
    ),
  )
  if (selfIntersect(geometry)) {
    errors.push('has-self-intersection')
...
function validatePolygonRings(polygonRings) {
  ...
  const errors = []
  errors.push(...flatten(polygonRings.map(ring => polygonRingIsValid(ring))))
...
function hasCrossingHoles(holes = []) {
  ...
  let i = 0
  let j = 0
  while (ok && (i !== length - 1 || j !== length - 1)) {
