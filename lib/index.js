...
async function parse(input, options = {}) {
  ...
  const targetSrs = options.targetSrs || WGS_84

  ctx.proj = createProj(ctx.srsCode, targetSrs)
  ...
  for (const voie of (layers.ZONCOMMUNI || [])) {
    voie.extraProperties = {labels: extractLabels(voie, ctx)}
  }

  for (const numeroVoie of (layers.NUMVOIE || [])) {
  ...
  for (const subdfisc of (layers.SUBDFISC || [])) {
...
async function parseSrs(input, options = {}) {
  const ctx = createModel({
    indexedItems: await parseInput(input),
    bundle: options.bundle,
    overrideSrsCode: options.overrideSrsCode,
