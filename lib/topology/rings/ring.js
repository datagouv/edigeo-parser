const {first, last, tail} = require('lodash')
const isClockwise = require('@turf/boolean-clockwise')
const debug = require('debug')('edigeo-parser')
const {positionIsEqual, getArcPosition, pathIsClosed} = require('./util')

class Ring {

  constructor(face, mode = 'TOP') {
  ...
  toObject() {
    ...
    // TOP
    const clockwise = isClockwise(this.coordinates)
    const isExterior = (isClockwise && this.relativePosition === 'right') ||
